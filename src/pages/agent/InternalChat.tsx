...
const InternalChat = () => {
  ...
  const getChat = useCallback(async () => {
    const res = await InternalChatService.createInternalChat(id);
    setChat(res.data);
  ...
  const handleSend = useCallback(
    async (message: { type: string; content: any }) => {
      await InternalChatService.sendMessage(message, chat.id || "");
    },
    [chat.id]
  );

  const onFetchMore = async () => {
    const res = await InternalChatService.createInternalChat(id, page + 1);
    setPage((prev) => prev + 1);
    setMessages((prev) => ({
      ...prev,
      ...res.messages,
